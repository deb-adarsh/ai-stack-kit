...
**`init` client hint (optional):** the CLI inspects **your home directory only** (not the repo’s `.idea` or which window is focused). **`sync` / `apply` always follow `spec.yaml` → `client.type`**. **Rough order:** (1) **VS Code ecosystem** — if **`~/.vscode`** or **`~/.copilot`** exists: **`~/.copilot`** → **`copilot`** (GitHub Copilot); else **`~/.claude`** → **`claude`** (Claude Code, commonly used from VS Code); else default **`copilot`** for a VS Code–style install; (2) **`~/.cursor`** → **`cursor`**; (3) **`~/.claude`** (standalone, no VS Code markers matched earlier); (4) JetBrains paths under home. Override in the init prompt anytime.
